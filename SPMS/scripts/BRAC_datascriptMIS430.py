...
course = Course_T(courseID='MIS430', courseName="Data Communication and Networking", numOfCredits=4, program_id=10,
                  courseType="Core")
course.save()

# CO
plolist = list(PLO_T.objects.filter(program_id=10))
...
instructors = []
instructors.append(Instructor_T.objects.get(pk=5401))
instructors.append(Instructor_T.objects.get(pk=5402))
instructors.append(Instructor_T.objects.get(pk=5403))

dept = Department_T.objects.get(pk=10)
program = Program_T.objects.get(pk=10)


def updatedatabase(d, sem, y):

    df = pd.read_excel("MIS430_BRAC.xlsx", sheet_name="Marks")
