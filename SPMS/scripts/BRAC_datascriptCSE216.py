...
course = Course_T(courseID='CSE216', courseName="Microprocessor, Interfacing and Assembly Language",
                  numOfCredits=4, program_id=7, courseType="Core")
course.save()

# CO
plolist = list(PLO_T.objects.filter(program_id=7))
...
instructors = []
instructors.append(Instructor_T.objects.get(pk=5104))
instructors.append(Instructor_T.objects.get(pk=5105))
instructors.append(Instructor_T.objects.get(pk=5106))

dept = Department_T.objects.get(pk=7)
program = Program_T.objects.get(pk=7)


def updatedatabase(d, sem, y):

    df = pd.read_excel("CSE216_BRAC.xlsx", sheet_name="Marks")
