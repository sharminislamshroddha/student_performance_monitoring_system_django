...
course = Course_T(courseID='ECN202', courseName="Principles of Macroeconomics",
                  numOfCredits=3, program_id=8, courseType="Core")
course.save()

# CO
plolist = list(PLO_T.objects.filter(program_id=8))
...
instructors = []
instructors.append(Instructor_T.objects.get(pk=5204))
instructors.append(Instructor_T.objects.get(pk=5205))
instructors.append(Instructor_T.objects.get(pk=5206))

dept = Department_T.objects.get(pk=9)
program = Program_T.objects.get(pk=8)


def updatedatabase(d, sem, y):

    df = pd.read_excel("ECN202_BRAC.xlsx", sheet_name="Marks")
