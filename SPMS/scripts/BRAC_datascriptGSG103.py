...
course = Course_T(courseID='GSG103', courseName="Theories and Methods of Inter-disciplinary Studies", numOfCredits=3, program_id=12,
                  courseType="Core")
course.save()

# CO
plolist = list(PLO_T.objects.filter(program_id=12))
...
instructors = []
instructors.append(Instructor_T.objects.get(pk=5601))
instructors.append(Instructor_T.objects.get(pk=5602))
instructors.append(Instructor_T.objects.get(pk=5603))

dept = Department_T.objects.get(pk=12)
program = Program_T.objects.get(pk=12)


def updatedatabase(d, sem, y):

    df = pd.read_excel("GSG103_BRAC.xlsx", sheet_name="Marks")
