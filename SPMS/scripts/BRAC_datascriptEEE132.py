...
course = Course_T(courseID='EEE132', courseName="Introduction to Materials and Chemistry",
                  numOfCredits=4, program_id=9, courseType="Foundation")
course.save()

# CO
plolist = list(PLO_T.objects.filter(program_id=9))
...
instructors = []
instructors.append(Instructor_T.objects.get(pk=5301))
instructors.append(Instructor_T.objects.get(pk=5302))
instructors.append(Instructor_T.objects.get(pk=5303))

dept = Department_T.objects.get(pk=8)
program = Program_T.objects.get(pk=9)


def updatedatabase(d, sem, y):

    df = pd.read_excel("EEE132_BRAC.xlsx", sheet_name="Marks")
