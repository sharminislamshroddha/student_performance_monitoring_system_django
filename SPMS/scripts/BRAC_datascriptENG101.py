...
course = Course_T(courseID='ENG101', courseName="Listening & Speaking English", numOfCredits=3, program_id=11,
                  courseType="Core")
course.save()

# CO
plolist = list(PLO_T.objects.filter(program_id=11))
...
instructors = []
instructors.append(Instructor_T.objects.get(pk=5504))
instructors.append(Instructor_T.objects.get(pk=5505))
instructors.append(Instructor_T.objects.get(pk=5506))

dept = Department_T.objects.get(pk=11)
program = Program_T.objects.get(pk=11)


def updatedatabase(d, sem, y):

    df = pd.read_excel("ENG101_BRAC.xlsx", sheet_name="Marks")
